<script lang="ts">
  ...
  // Sync state when parent changes the prop (e.g. sidebar brand click)
  $effect(() => { if (initialBrand !== activeBrand) activeBrand = initialBrand })
  $effect(() => { if (initialType  !== activeType)  activeType  = initialType  })
  $effect(() => {
    carouselIdx = 0
  ...
  function chipStyle(isActive: boolean) {
    return isActive
      ? ($isDark
          ? 'background:rgba(51,78,139,0.60);border:1px solid rgba(51,78,139,0.80);color:white;box-shadow:0 4px 15px rgba(51,78,139,0.40);transform:scale(1.04);backdrop-filter:blur(10px);'
          : 'background:rgba(51,78,139,0.15);border:1px solid rgba(51,78,139,0.40);color:#334E8B;box-shadow:0 4px 15px rgba(51,78,139,0.12);transform:scale(1.04);backdrop-filter:blur(10px);')
      : ($isDark
          ? 'background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.10);color:rgba(255,255,255,0.45);backdrop-filter:blur(10px);'
          : 'background:rgba(255,255,255,0.72);border:1px solid rgba(100,130,220,0.20);color:rgba(20,30,80,0.50);backdrop-filter:blur(10px);')
  }
  ...
  <div class="max-w-7xl mx-auto">
    ...
    <div class="flex flex-col sm:flex-row sm:items-end justify-between mb-6 md:mb-8 gap-3">
      ...
      <div class="flex items-center gap-3">
        ...
        <a href="https://www.vapsarioverde.com.mx/" class="flex items-center gap-1.5 text-xs transition-colors hover:text-blue-600" style="color:{seeAllColor}">
    ...
    <!-- Filter panel -->
    <div class="mb-6 md:mb-8 rounded-2xl p-3 md:p-4 space-y-2.5 md:space-y-3" style={filterPanelStyle}>
      ...
      <div class="flex items-center gap-2 overflow-x-auto scrollbar-none">
        ...
        {#each TYPES as type (type)}
          <button onclick={() => activeType = type}
            class="flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-medium transition-all duration-200"
    ...
    {#if viewMode === 'grid'}
      ...
      {:else}
              ...
              <div class="flex items-center justify-between mt-4 px-1">
                ...
                <div class="flex items-center gap-1.5">
                  {#each vehicles as _, i (i)}
                    <button onclick={() => goTo(i)} class="transition-all duration-300"
        ...
        <!-- Desktop bento -->
        {#if featured}
          <div class="hidden md:grid gap-4" style="grid-template-columns:repeat(4,1fr);grid-auto-rows:280px;">
...
{#snippet VehicleCard({ vehicle, featured = false }: { vehicle: typeof ALL_VEHICLES[0], featured?: boolean })}
  ...
  {@const typeBadge = `background:${isDarkVal ? 'rgba(0,0,0,0.50)' : 'rgba(255,255,255,0.80)'};border:${isDarkVal ? '1px solid rgba(255,255,255,0.16)' : '1px solid rgba(100,140,220,0.28)'};color:${isDarkVal ? 'rgba(255,255,255,0.72)' : 'rgba(20,30,80,0.68)'};backdrop-filter:blur(12px);box-shadow:${isDarkVal ? 'inset 0 1px 0 rgba(255,255,255,0.12)' : 'inset 0 1px 0 rgba(255,255,255,0.90)'};`}
  <div class="h-full" style="position:relative;">
    <LiquidGlass
      variant="card"
      class="relative overflow-hidden cursor-pointer h-full"
    >
      <div class="relative overflow-hidden" style="height:{featured ? '65%' : '58%'};min-height:{featured ? 220 : 160}px;">
        <img src={vehicle.img} alt="{vehicle.brand} {vehicle.model} {vehicle.year}"
          class="w-full h-full object-cover transition-transform duration-700" />
        <div class="absolute inset-0" style="background:linear-gradient(to bottom,rgba(0,0,0,0.06) 0%,rgba(5,8,20,0.80) 100%)"></div>
        <div class="absolute top-3 right-3 px-2 py-0.5 rounded-full text-xs" style={typeBadge}>
          {TYPE_ICONS[vehicle.type as VehicleType]} {vehicle.type}
        </div>
        {#if vehicle.badge}
          <div class="absolute top-3 left-3 px-2 py-1 rounded-lg text-xs font-medium text-white"
            style="background:rgba(51,78,139,0.88);border:1px solid rgba(255,255,255,0.22);backdrop-filter:blur(10px);">
            {vehicle.badge}
          </div>
        {/if}
        <div class="absolute bottom-3 left-3 right-3">
          <p class="text-white/52 text-xs uppercase tracking-widest">{vehicle.brand}</p>
          <h3 class="text-white font-bold" style="font-size:{featured ? '1.4rem' : '1.05rem'};line-height:1.2;">
            {vehicle.model} <span class="text-white/60">{vehicle.year}</span>
          </h3>
        </div>
      </div>
      <div class="p-4 pb-5" style="border-top:{isDarkVal ? '1px solid rgba(255,255,255,0.07)' : '1px solid rgba(100,140,220,0.12)'};">
        <p class="text-xs mb-1 font-medium tracking-wider" style="color:{isDarkVal ? 'rgba(255,255,255,0.50)' : 'rgba(20,30,80,0.50)'}">{vehicle.version}</p>
        <p class="text-xs leading-relaxed mb-4" style="color:{isDarkVal ? 'rgba(255,255,255,0.72)' : 'rgba(20,30,80,0.72)'}">
          <Tag size={10} style="display:inline;margin-right:4px;color:#3b82f6;" />
          {vehicle.deal}
        </p>
        <a href={vehicle.link} class="inline-flex items-center gap-1.5 text-xs font-medium transition-all duration-200 hover:gap-2"
          style="color:{isDarkVal ? 'rgba(100,150,255,0.9)' : '#334E8B'}">
